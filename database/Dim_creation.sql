...
CREATE TABLE dim_customer (
    ...
    address_status VARCHAR(30),  -- current / old / etc
...
SELECT @StartDate = '1/1/2020'  -- Set The start and end date 
 , @EndDate = '31/12/2025'--Non inclusive. Stops on the day before this.
...
CREATE TABLE dim_shipping (
    shipping_sk INT IDENTITY(1,1) PRIMARY KEY,  -- Surrogate Key

    method_id INT NOT NULL,                      -- Business Key
...
CREATE TABLE dim_book (
    book_sk INT IDENTITY(1,1) PRIMARY KEY,   -- Surrogate Key

    book_id INT NOT NULL,                   -- Business Key (from OLTP)
...
CREATE TABLE dim_orderstatus (
    ...
    status_id INT NOT NULL,                   -- Business Key (from OLTP)

    status_value VARCHAR(20)                -- e.g. Pending, Shipped, Cancelled
);
GO
ALTER TABLE dim_orderstatus
add  ssc bit;
...
CREATE TABLE dim_author (
    author_sk INT IDENTITY(1,1) PRIMARY KEY,   -- Surrogate Key

    author_id INT NOT NULL,                   -- Business Key (from OLTP)
...
CREATE TABLE bridge_book_author (
    ...
    author_weighting_factor DECIMAL(5,2) NOT NULL,  -- contribution weight (0-1 or %)
...
ADD CONSTRAINT fk_book_sk
FOREIGN KEY (book_sk)
REFERENCES dim_book(book_sk);


ALTER TABLE bridge_book_author
ADD CONSTRAINT fk_author_sk
FOREIGN KEY (author_sk)
REFERENCES dim_author(author_sk);
...
-- استعلام لمعرفة النطاق الزمني للبيانات في المصدر
SELECT 
    MIN(order_date) AS Min_Date_In_OLTP, 
    MAX(order_date) AS Max_Date_In_OLTP,
...
ALTER TABLE fact_orderline DROP CONSTRAINT FK_fact_date; 


ALTER TABLE fact_orderline 
ADD CONSTRAINT FK_fact_date FOREIGN KEY (date_sk) REFERENCES DimDate(DateSK);

TRUNCATE TABLE fact_orderline;

ALTER TABLE fact_orderline 
WITH CHECK ADD CONSTRAINT FK_fact_date 
...
SELECT @StartDate = '1/1/2020'  -- Set The start and end date 
 , @EndDate = '12/31/2025'--Non inclusive. Stops on the day before this.
 , @CurrentMonth = 1 --Counter used in loop below.
